...
const GUIDANCE = {
  general: '',
  case_study: `- STRUCTURE (case study): the FIRST H2 must be the situation/problem before the ORIGINAL ELEMENT's result happened (the real starting point — what was broken/missing) — do NOT make the stat/result its own H2 before this; the "da-stat-callout" belongs inline in the intro paragraph (before the first H2), never as a heading of its own. The SECOND H2 covers what was actually done (the specific approach/decisions). The THIRD H2 is the result itself, built around the ORIGINAL ELEMENT's real number/outcome. Remaining H2s can cover why it worked or what a reader can copy from it. Do NOT write this as a generic tips listicle — every section must trace back to this one real story.`,
